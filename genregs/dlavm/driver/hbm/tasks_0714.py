...
def MVMBasic(**kwargs):
    ...
    if full:
        ...
        def for_mvm_single(k):
            ...
            CSB_Write(regs, 10,DAT_IN_BASE_ADDR + k * split_token * Pixel_Data_Bytes)
            CSB_Write(regs, 11,HBM00_WT_BASE_ADDR)
            CSB_Write(regs, 12,wt_size_in_bits)
            CSB_Write(regs, 13,feature_out_base + k * split_token * Pixel_Data_Bytes)
            ...
            CSB_Write(regs, 27,Res_Add_BASE_ADDR + If(mode==MVMBNRES_MODE or mode==MVMBNRESARG_MODE, k * split_token * Pixel_Data_Bytes, 0))
            CSB_Write(regs, 28,0)
