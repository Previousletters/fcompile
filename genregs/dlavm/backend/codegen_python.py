    ...
    def gen_accel(self, node):
        ...
        for reg in node["csb_regs"]:
            if reg[0] == 1:
                ...
                tp_func_inits.append(f"{self.tab}CSB_Write({reg[1]}, {data})")
            elif reg[0] == 0:
                tp_func_inits.append(f"{self.tab}while(CSB_Read({reg[1]}) != {reg[2]}):\n{self.tab}{self.tab}pass")
...
from ffi import CSB_Write, CSB_Read
%(storages)s
