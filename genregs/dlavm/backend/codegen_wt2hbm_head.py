...
class CodeGenWt2HbmHead(CodeGenTestHeadOps):
    ...
    def gen_wt2hbm(self, enum_name, hbm_addr, total_bytes, log2_Bank_Step=28):
        ...
        rt_address = self.storage.get_address("runtime0", 0)
        ddr_address = rt_address & 0xffffffff
        tp_source = [f"void wt2hbm_{enum_name} (HANDLE device, HANDLE h2cx, char* path)" + " {"]
        tp_source.append(f"{self.tab}char real_path[100];")
        tp_source.append(f"{self.tab}for (int port = 0; port < 32; port++)" + " {")
        tp_source.append(f"{self.tab}{self.tab}sprintf(real_path, path, port);")
        ...
        if split_times == 1:
            ...
            tp_source.append(f"{self.tab}{self.tab}CSB_Write(device, 64+5, hbm_out_addr & 0xffffffff);")
            tp_source.append(f"{self.tab}{self.tab}CSB_Write(device, 64+6, hbm_out_addr >> 32);")
            ...
            tp_source.append(f"{self.tab}{self.tab}{self.tab}CSB_Write(device, 64+5, hbm_out_addr & 0xffffffff);")
            tp_source.append(f"{self.tab}{self.tab}{self.tab}CSB_Write(device, 64+6, hbm_out_addr >> 32);")
            tp_source.append(f"{self.tab}{self.tab}{self.tab}CSB_Write(device, 64+7, (i == 0) ? {total_bits_in_each_slice} : {last_split_bits});")
            ...
            tp_source.append(self.tab + self.tab + self.tab + "}")
