...
for (int i = 0; i < 1000; i=i+1) {
#endif
  CSB_Write(device, 194, 12355584); // wt_base_addr
  CSB_Write(device, 195, (kvcache ? (11307008 + ((token - 1) * 64)) : 11307008)); // in_addr
  CSB_Write(device, 196, 32768); // in_head_stride
  CSB_Write(device, 197, 8192); // in_line_stride
  CSB_Write(device, 198, (kvcache ? (12421120 + ((token - 1) * 64)) : 12421120)); // out_addr
  CSB_Write(device, 199, 32768); // out_head_stride
  CSB_Write(device, 200, 8192); // out_line_stride
  CSB_Write(device, 201, 4); // (CHin + Tout - 1) / Tout
  CSB_Write(device, 202, token);
  CSB_Write(device, 203, 2); // Weight Head
  CSB_Write(device, 204, (kvcache ? 1 : token)); // TRP_Feature_Win
  CSB_Write(device, 205, 32); // Feature Head
  CSB_Write(device, 206, 15); // TRP_Feature_Repeat_times_minus1
  CSB_Write(device, 207, 11688); // FP16 rsqrt
  CSB_Write(device, 208, 0); // out to BRAM, in from BRAM
  CSB_Write(device, 209, 2); // TRP start
  while(CSB_Read(device, 193) != 1) {}
#ifdef REGS_DEBUG
}
QueryPerformanceCounter(&stop_run);
...
void step338 (HANDLE& device, int kvcache, int token) {
// accel.hbm.mvm_bn accel operator node, storage data in runtime3 with 0 offset
#ifdef REGS_DEBUG
LARGE_INTEGER start_run;
LARGE_INTEGER stop_run;
...
void chatglm_dynamic_control_0422_2029(HANDLE& device, int token, int kvcache) {
