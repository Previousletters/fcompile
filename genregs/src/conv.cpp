...
HBM_DLL struct MVM_Cfg Get_MVM_Cfg(int Hin, int Win, int CHin, int CHout, int in_feature_dw, int wt_dw, int wt_scale_dw)
{
    ...
    printf("%d, %d, %d, %d, %d\n", dat_num_per_row, CHout_Split_Times, CHout_Padding, WT_CHin_div_Tin, Hout_Split_Times);
...
#ifdef Run_on_FPGA
void FPGA_MVM(struct MVM_Cfg mvm_cfg, int relu_en, struct Mapped_Feature* feature_in, struct Mapped_Weight* wt, struct Mapped_Feature* feature_out, int Out_to_BRAM, int In_from_BRAM, int out_base_addr, int in_base_addr, HANDLE device, int log2_WT_base_addr_Bank_Step, int Left_WT_Base_Addr)
{
        ...
        for (int n = 0; n < mvm_cfg.CHout_Split_Times; n = n + 1)
            for (int k = 0; k < mvm_cfg.Hout_Split_Times; k = k + 1)
            {
                ...
                FPGA_MVM_single_time(CH_in_single,in_height_single,mvm_cfg.Win,CH_out_single,mvm_cfg.Wout,Hout_single,
                        ...
                        mvm_cfg.best_single_dat_banks,0,dma_wt_reuse_single, 8, 0, device);
            }
    }
        ...
        for (int k = 0; k < mvm_cfg.Hout_Split_Times; k = k + 1)
            for (int n = 0; n < mvm_cfg.CHout_Split_Times; n = n + 1)
            {
                ...
                FPGA_MVM_single_time(CH_in_single,in_height_single,mvm_cfg.Win,CH_out_single,mvm_cfg.Wout,Hout_single,
                        ...
                        mvm_cfg.best_single_dat_banks,dma_dat_reuse_single,0, 8, 0, device);
            }
        CSB_Finish();
    }
}
    ...
    int single_dat_buf_num, int dma_dat_reuse, int dma_wt_reuse, int log2_WT_base_addr_Bank_Step, int Left_WT_Base_Addr, HANDLE device)
{
    ...
    CSB_Write(device, 0, 0b011111);//Kick off the run. MVM Path: fsm_wt,fsm_dat,buf_init,dma_wt,dma_dat
    CSB_Write(device, 64+0,0b0001);//{ResAdd, BN, wdma}
    
    CSB_Read(device, 64 + 19, 1);
}

void FPGA_RunHBM_FC(struct Mapped_Feature* feature_in, struct Mapped_Weight* wt, struct Mapped_Feature* feature_out, int Out_to_BRAM, int In_from_BRAM, int out_base_addr, int in_base_addr, HANDLE device, int log2_WT_base_addr_Bank_Step, int Left_WT_Base_Addr)
{
    int current_in_base_addr, current_out_base_addr;
    current_in_base_addr =(In_from_BRAM==1)?in_base_addr:((uint64_t)feature_in->payload);
    current_out_base_addr=( Out_to_BRAM==1)?out_base_addr:((uint64_t)feature_out->payload);
    ...
    CSB_Write(device, 64 + 6, ((out_ch_slice + Tout - 1) / Tout)); //CSB_Write(device, 64+6,((CHout+Tout-1)/Tout));
    CSB_Write(device, 64+20,current_out_base_addr);   
    CSB_Write(device, 64+23,CHout_Split_Times_minus1);
    CSB_Write(device, 0,0b111111);//Conv Path: FC_mode,fsm_wt,fsm_dat,buf_init,dma_wt,dma_dat
