...
HBM_DLL struct HBM_MVM_Cfg Get_HBM_MVM_Cfg(int Hin, int Win, int CHin, int CHout, int in_feature_dw, int wt_dw, int wt_scale_dw)
{
    ...
    //min_wt_depth=Tout*WT_CHin_div_Tin/HBM_Port;
    ...
    ret.CHin_Padding_with_Tin = WT_CHin_Padding_with_Tin;
    ret.Hin = Hin;
    ret.Win = Win;
...
void Save_HBM_MVM_Cfg(const char* cfg_name, struct HBM_MVM_Cfg mvm_cfg, FILE* fp)
{

    fprintf(fp, "struct HBM_MVM_Cfg %s={%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d};\n", cfg_name,
...
void run_FPGA_HBM_FC(struct HBM_MVM_Cfg mvm_cfg, int relu_en, struct Mapped_Feature* feature_in, struct Mapped_Weight* wt, struct Mapped_Feature* feature_out, int Out_to_BRAM, int In_from_BRAM, int out_base_addr, int in_base_addr,HANDLE user_device)
{
    ...
    if (mvm_cfg.method == 0)//Hout_Split_Times->CHout_Split_Times
    {
        for (int n = 0; n < mvm_cfg.CHout_Split_Times; n = n + 1)
            for (int k = 0; k < mvm_cfg.Hout_Split_Times; k = k + 1)
            {
                ...
                run_FPGA_HBM_FC_single_time(CH_in_single,in_height_single,mvm_cfg.Win,CH_out_single,mvm_cfg.Wout,Hout_single,
                                        Out_to_BRAM, In_from_BRAM, relu_en,
                        (current_in_base_addr) + feature_in->line_stride * line_offset_in, 
                        feature_in->surface_stride, 
                        feature_in->line_stride,
                        ...
                        (current_out_base_addr) + feature_out->line_stride * line_offset_out + feature_out->surface_stride * n * (mvm_cfg.out_ch_slice / Tout), 
                        feature_out->surface_stride, 
                        feature_out->line_stride,
                        
                        mvm_cfg.best_single_dat_banks,0,dma_wt_reuse_single, user_device);
            }
    }
        ...
        for (int k = 0; k < mvm_cfg.Hout_Split_Times; k = k + 1)
            for (int n = 0; n < mvm_cfg.CHout_Split_Times; n = n + 1)
            {
                ...
                run_FPGA_HBM_FC_single_time(CH_in_single,in_height_single,mvm_cfg.Win,CH_out_single,mvm_cfg.Wout,Hout_single,
                        ...
                        mvm_cfg.best_single_dat_banks,dma_dat_reuse_single,0, user_device);
    ...
    int single_dat_buf_num, int dma_dat_reuse, int dma_wt_reuse, HANDLE user_device)
{
    ...
    //cfg conv_path
    CSB_Write(user_device, 7, CH_in_div_Tout);
    CSB_Write(user_device, 8, CH_in_div_Tin);
    CSB_Write(user_device, 31, CH_in_res_Tin_div_Tout_minus1);
    CSB_Write(user_device, 19, CH_out_div_Tout);
	
    CSB_Write(user_device, 20, feature_in_base);
    CSB_Write(user_device, 23, wt_size_in_bits);
    CSB_Write(user_device, 32, wt_num_div_Tin);
    CSB_Write(user_device, 24, wt_base_addr);
    CSB_Write(user_device, 49, log2_WT_base_addr_Bank_Step); // #TODO!!!
    CSB_Write(user_device, 25, (dma_wt_reuse << 1) | dma_dat_reuse);
    CSB_Write(user_device, 47, (Out_to_BRAM << 1) | In_from_BRAM);//MVM_Out_to_BRAM_mode_r<=wdata[1], MVM_In_from_BRAM_mode_r<=wdata[0]

    CSB_Write(user_device, 64+6,CH_out_div_Tout);
    CSB_Write(user_device, 64+20,feature_out_base);

    CSB_Write(user_device, 0, 0b111111);//Kick off the run. MVM Path:FC_mode,fsm_wt,fsm_dat,buf_init,dma_wt,dma_dat
    CSB_Write(user_device, 64+0,0b1001);//{FC_mode,ResAdd, BN, wdma}
    //CSB_Read(user_device, 64+19);
    CSB_Read(user_device, 64+19, 1);
    CSB_Write(user_device, 47,0);//clear
}
#endif
#endif
