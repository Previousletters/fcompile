...
void FPGA_Run_Softmax(struct Mapped_Feature* feature_in, struct Mapped_Feature* feature_out,int pixel_in, int Softmax_Out_and_In_Mode, HANDLE device)
{
...
//	printf(" \n reg 23=%d",pixel_out);//pixel_out
    ...
    CSB_Write(device, Matrix_reg_bias+19,pixel_in);//pixel_in
	CSB_Write(device, Matrix_reg_bias+23,feature_in->channel);//pixel_out
	//Kick of the run
	CSB_Write(device, Matrix_reg_bias+0,0b00001000);//Ln_start
    
    CSB_Read(device, Matrix_reg_bias + 1, 1);
    CSB_Finish();
}


void FPGA_Run_Activation(unsigned short fp16_wt[], unsigned short fp16_bias[], unsigned short fp16_x_region[], struct Mapped_Feature* feature_in, struct Mapped_Feature* feature_out, int pixel_in, int Activation_Out_and_In_Mode, HANDLE device)
{
	int parameter_addr;
    int width = ((feature_in->width + AXI_BURST_LEN_SOFTMAX - 1) / AXI_BURST_LEN_SOFTMAX) * AXI_BURST_LEN_SOFTMAX;
    int dat_in_surface_stride = Pixel_Data_Bytes * feature_in->height * width;
...
//    printf(" \n reg 19=%d\n",pixel_in);//pixel_in
    ...
    CSB_Write(device, Matrix_reg_bias+19,pixel_in);//pixel_in
    CSB_Write(device, Matrix_reg_bias+26,Activation_Out_and_In_Mode);
    CSB_Write(device, Matrix_reg_bias+29,0b110000000000000000);//parameters start
    for(int i=0;i<47;i++)
    {
        parameter_addr=i+1; 
        if(i<16)
            CSB_Write(device, Matrix_reg_bias+29,(parameter_addr<<18)+(1<<16)+fp16_wt[i]); //FP16_wt
        else 
            if(i<32)
                CSB_Write(device, Matrix_reg_bias+29,(parameter_addr<<18)+(1<<16)+fp16_bias[i-16]); //FP16_wt
            else
                CSB_Write(device, Matrix_reg_bias+29,(parameter_addr<<18)+(1<<16)+fp16_x_region[i-32]); //FP16_wt
    }
    CSB_Write(device, Matrix_reg_bias+29,0b100000000000000000);//parameters done
 
	//Kick of the run
	CSB_Write(device, Matrix_reg_bias+0,0b000010000);//Ln_start
...
//    printf(" \n reg19= %d",feature_in->width);//pixel_in
    ...
    CSB_Write(device, Matrix_reg_bias+18,fp20_data);

    CSB_Write(device, Matrix_reg_bias+19,feature_in->width);//pixel_in
    CSB_Write(device, Matrix_reg_bias+28,LN_CH_burst_times_minus1);
    CSB_Write(device, Matrix_reg_bias+30,Layer_Norm);
    
	//Kick of the run
	CSB_Write(device, Matrix_reg_bias+0,0b00100000);//Ln_stage0_start
    CSB_Read(device, Matrix_reg_bias + 1, 1);
	
	CSB_Write(device, Matrix_reg_bias+26,LN_Out_and_In_Mode);//LN_Out_and_In_Mode
	CSB_Write(device, Matrix_reg_bias+0,0b01000000);//Ln_stage1_start
    CSB_Read(device, Matrix_reg_bias + 1, 1);
    CSB_Finish();
}

void FPGA_Run_Transpose(struct Mapped_Feature* feature_in, struct Mapped_Weight* feature2weight_out, int Transpose_Out_and_In_Mode, HANDLE device)
{
...
//    printf(" \n reg 25=%d",wt_num_div_Tout);//wt_num_div_Tout
    ...
    CSB_Write(device, Matrix_reg_bias+11,Tout);//feature_in->width;
    CSB_Write(device, Matrix_reg_bias+19,pixel_in);//pixel_in
    CSB_Write(device, Matrix_reg_bias+23,in_ch);//pixel_out
    CSB_Write(device, Matrix_reg_bias+25,wt_burst_times);//wt_num_div_Tout
	CSB_Write(device, Matrix_reg_bias+26,Transpose_Out_and_In_Mode);//LN_Out_and_In_Mode
    ...
    for (int chout=0; chout<CHout_div_Tout;chout=chout+1)
        for (int chin=0; chin<WT_CHin_div_Tblock;chin=chin+1)
        {
            ...
            CSB_Write(device, Matrix_reg_bias+3 ,in_base_addr);//feature_in_base_addr
            CSB_Write(device, Matrix_reg_bias+6 ,dat_out_base_addr);//feature_out_base_addr
            //Kick of the run
	        CSB_Write(device, Matrix_reg_bias+0,0b00000010);//Transpose_start
