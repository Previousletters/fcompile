...
def chatglm_block_dynamic(data, pos_weight, silu_weight, token, index, kvcache, last_token=0):
...
def chatglm_main_dynamic(debug, block_size=28, device=dlavm.device.HBM0321, aux_cfg=0, wt2hbm=0):
    ...
    for n in range(block_size):
        data = chatglm_block_dynamic(data, pos_weight, silu_weight, token, n, kvcache=kvcache, last_token=last_token)
    ...
    if aux_cfg:
        ...
        cmd_path = os.path.join("test", name + ".bin")
    ...
    log_path = os.path.join("test", name + ".log")
...
if __name__ == "__main__":
    ...
    chatglm_main_dynamic(debug=1, block_size=28, device=device, aux_cfg=0, wt2hbm=1)
