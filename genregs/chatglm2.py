...
def chatglm_main_dynamic(block_size=28, device=dlavm.device.HBM0321, aux_cfg=0, wt2hbm=0, py=0, clock=0, full=0, path_prefix = "BLOCK_write_data"):
    ...
    if aux_cfg:
        if wt2hbm:
            expr, source, storage, _, params, debug_params = backend.cfg_wt2hbm(output, name, addr_assign_aux)
        else:
            expr, source, storage, _, params, debug_params = backend.cfg_head(output, name, addr_assign_aux)
        ...
        expr, source, storage, _ = backend.csb_wt2hbm_head(output, name, addr_assign)
    elif py:
        expr, source, storage, _ = backend.csb_python(output, name, addr_assign)
    elif clock:
        expr, source, storage, _ = backend.csb_test_clock_ops(output, name, addr_assign)
    else:
        expr, source, storage, _ = backend.csb_test_head_ops(output, name, addr_assign)
    if py:
