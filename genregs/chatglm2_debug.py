...
def chatglm_block_dynamic(data, pos_weight, silu_weight, token, index, kvcache, last_token, path_prefix="BLOCK_write_data"):
    ...
    qk_data = adr.reshape(qk_data, [34, token, 128])
...
def kvmvm_main_dynamic(device=dlavm.device.HBM0321, aux_cfg=0, wt2hbm=0, clock=0, py=0, path_prefix = "BLOCK_write_data"):
    sys.setrecursionlimit(3000)  # 将默认的递归深度修改为3000
    token = ne.Var("token", device.MAX_TOKEN)
    cache = ne.Var("cache", device.MAX_TOKEN)
    name = f"kvmvm_{device.MAX_TOKEN}"
    name += "_aux" if aux_cfg else ""
    name += "_clock" if clock else ""
    name += "_wt2hbm" if wt2hbm else ""
    name += "_" + strftime('%m%d_%H%M', localtime())
    data = adr.hbm.var_ddr("data_in", [token, 128])
    weight0 = adr.hbm.const_hbm("weight0", None, [128, cache])
    weight1 = adr.hbm.const_hbm("weight1", None, [cache, 128])
...
if __name__ == "__main__":
    ...
    kvmvm_main_dynamic(device=device, aux_cfg=args.aux, wt2hbm=args.wt2hbm, py=args.py, path_prefix=args.prefix)
