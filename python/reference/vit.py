...
from quan.quant_sparse_layers import QuanSparseConv2d, QuanSparseLinear, QuanSparseConv2dSRP, QuanSparseLinearSRP
#from quan.I_bert import LsqQuan_gelu,LsqQuan_layernorm,LsqQuan_softmax
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.fc1 = QuanSparseLinearSRP(in_features, hidden_features, sparsity=0.0)
        self.act = act_layer()
        #self.fc2 = nn.Linear(hidden_features, out_features)
        self.fc2 = QuanSparseLinearSRP(hidden_features, out_features, sparsity=0.0)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        self.qkv = QuanSparseLinearSRP(dim, dim * 3, bias=qkv_bias, sparsity=0.0)
        self.attn_drop = nn.Dropout(attn_drop)
        #self.proj = nn.Linear(dim, dim)
        self.proj = QuanSparseLinearSRP(dim, dim, sparsity=0.0)
        self.proj_drop = nn.Dropout(proj_drop)
        #self.softmax = LsqQuan_softmax()
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if representation_size and not distilled:
            ...
            self.pre_logits = nn.Sequential(OrderedDict([
                ...
                ('fc', QuanSparseLinearSRP(embed_dim, representation_size, sparsity=0.0)),
                ('act', nn.Tanh())
            ]))
        else:
        ...
        self.head = QuanSparseLinearSRP(self.num_features, num_classes, sparsity=0.0) if num_classes > 0 else nn.Identity()
        self.head_dist = None
        if distilled:
            #self.head_dist = nn.Linear(self.embed_dim, self.num_classes) if num_classes > 0 else nn.Identity()
            self.head_dist = QuanSparseLinearSRP(self.embed_dim, self.num_classes, sparsity=0.0) if num_classes > 0 else nn.Identity()
    ...
    def reset_classifier(self, num_classes, global_pool=''):
        ...
        self.head = QuanSparseLinearSRP(self.embed_dim, num_classes, sparsity=0.0) if num_classes > 0 else nn.Identity()
...
def _init_vit_weights(module: nn.Module, name: str = '', head_bias: float = 0., jax_impl: bool = False):
    ...
    * When called w/ valid n (module name) and jax_impl=True, will (hopefully) match JAX impl
    ...
    if isinstance(module, QuanSparseLinearSRP):
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = ''):
    ...
    if isinstance(model.head, QuanSparseLinearSRP) and model.head.bias.shape[0] == w[f'{prefix}head/bias'].shape[-1]:
        model.head.weight.copy_(_n2p(w[f'{prefix}head/kernel']))
        model.head.bias.copy_(_n2p(w[f'{prefix}head/bias']))
    #if isinstance(getattr(model.pre_logits, 'fc', None), nn.Linear) and f'{prefix}pre_logits/bias' in w:
    if isinstance(getattr(model.pre_logits, 'fc', None), QuanSparseLinearSRP) and f'{prefix}pre_logits/bias' in w:
