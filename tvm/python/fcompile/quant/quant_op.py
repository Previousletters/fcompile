...
class Quant(Function):
    ...
    def symbolic(g, tensor, bit_width, scale):
        ...
        return g.op("torchcustom::Quant", tensor, g_bit_width, g_scale)
...
class Dequant(Function):
    ...
    def symbolic(g, tensor, bit_width, scale_p):
        ...
        return g.op("torchcustom::Dequant", tensor, g_bit_width, g_scale)
...
class QuantifiedConv2d(nn.Conv2d):
    def __init__(self, *args, **kwargs):
        self.bit_width = _try_get_del(kwargs, "bit_width", 8)
        self.scale = _try_get_del(kwargs, "scale", [10])
        self.scale_weight = _try_get_del(kwargs, "scale_weight", [10])
        self.scale_bias = _try_get_del(kwargs, "scale_bias", [10])
        self.scale_out = _try_get_del(kwargs, "scale_out", [10])
        super(QuantifiedConv2d, self).__init__(*args, **kwargs)

    def forward(self, input):
        input_temp = Quant.apply(input, self.bit_width, self.scale)
        weight_temp = Quant.apply(self.weight, self.bit_width, self.scale_weight)
        bias_temp = Quant.apply(self.bias, self.bit_width, self.scale_bias)
        output = self._conv_forward(input_temp, weight_temp, bias_temp)
        #return output
        return Dequant.apply(output, self.bit_width, self.scale_out)
