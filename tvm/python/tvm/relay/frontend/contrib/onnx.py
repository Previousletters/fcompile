...
class GELU(OnnxOpConverter):
    ...
    @classmethod
    def _impl_v1(cls, inputs, attr, params):
        assert len(inputs) == 1, "GELU op take 1 inputs, {} given".format(len(inputs))
        return _op.accel.base.gelu(inputs[0])


class Quantize(OnnxOpConverter):
    """Operator converter for Pytorch ATen ops."""

    @classmethod
    def _impl_v1(cls, inputs, attr, params):
        assert len(inputs) == 3, "Quantize op take 3 inputs, {} given".format(len(inputs))
        bwidth = int(inputs[1].data.asnumpy())
        dscale = int(inputs[2].data.asnumpy())
        return _op.accel.base.quantize(inputs[0], bwidth, dscale)


class Dequantize(OnnxOpConverter):
    """Operator converter for Pytorch ATen ops."""

    @classmethod
    def _impl_v1(cls, inputs, attr, params):
        assert len(inputs) == 3, "Dequantize op take 3 inputs, {} given".format(len(inputs))
        bwidth = int(inputs[1].data.asnumpy())
        dscale = int(inputs[2].data.asnumpy())
        return _op.accel.base.dequantize(inputs[0], bwidth, dscale)
...
class LayerNorm(OnnxOpConverter):
    ...
    def _impl_v1(cls, inputs, attr, params):
        assert len(inputs) == 3, "Dequantize op take 3 inputs, {} given".format(len(inputs))
        return _op.nn.layer_norm(inputs[0], inputs[1], inputs[2])
...
def _get_convert_map(opset):
    return {
        ...
        # Custom operators
        "LayerNorm": LayerNorm.get_converter(opset),
        "Quant": Quantize.get_converter(opset),
        "Dequant": Dequantize.get_converter(opset),
