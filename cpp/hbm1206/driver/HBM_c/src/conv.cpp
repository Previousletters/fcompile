...
// WARNNING!!! 在testbench中，Win给的是定值，Hin (`Height+Win-1)/Win，在此情况下Skip如果不为1则会出现问题，需要查看解决
struct MVM_Cfg Get_MVM_Cfg(int Hin, int Win, int CHin, int CHout, int in_feature_dw, int wt_dw, int wt_scale_dw, int Skip_Factor)
{
    ...
    printf("Hout=%d, Wout=%d, CHin=%d\n", Hout, Wout, CHin);
    printf("in_height_first=%d, in_height_middle=%d, in_hight_last=%d\n", in_height_first, in_height_middle, in_height_last);
