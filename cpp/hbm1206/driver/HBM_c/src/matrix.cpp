...
void FPGA_Run_Activation(int parameters_addr, struct Mapped_Feature* feature_in, struct Mapped_Feature* feature_out, int Activation_Out_and_In_Mode, HANDLE device)
{
	...
	//Kick of the run
	CSB_Write(device, Matrix_reg_bias+0,0b010000);//Ln_start
...
void FPGA_Run_LN(struct Mapped_Feature* feature_in, struct Mapped_Feature* wt_and_bias, struct Mapped_Feature* feature_out, int Height, int RMS_Norm, int LN_Out_and_In_Mode, HANDLE device)
{
    ...
    CSB_Write(device, Matrix_reg_bias+19,Height);//pixel_in
	...
	CSB_Write(device, Matrix_reg_bias+0,0b0100000);//Ln_stage1_start
	while (CSB_Read(device, Matrix_reg_bias+1) != 1){	}
}
...
void FPGA_Run_Transpose(struct Mapped_Feature* feature_in, struct Mapped_Weight* feature2weight_out, int Transpose_Out_and_In_Mode, HANDLE device, int log2_WT_base_addr_Bank_Step, int Left_WT_Base_Addr)
{
	...
	CSB_Write(device, Matrix_reg_bias+0,0b010);
	while (CSB_Read(device, Matrix_reg_bias+1) != 1){	}
	//Kick of the run
}


void FPGA_Run_Feature2Weight(struct Mapped_Feature* feature_in, struct Mapped_Weight* weight_out, int Height, int Out_and_In_Mode, HANDLE device, int log2_WT_base_addr_Bank_Step, int Left_WT_Base_Addr)
{
    int width = ((feature_in->width + Tout - 1) / Tout) * Tout;
    int in_ch = ((feature_in->channel + T_quant_block - 1) / T_quant_block) * T_quant_block;
	int pixel_in,pixel_out,wt_burst_times,CHout_div_Tout, Block_wt_bits;
    ...
    CSB_Write(device, Matrix_reg_bias+8 ,Left_WT_Base_Addr);//feature_out_base_addr
    CSB_Write(device, Matrix_reg_bias+19,Height);//pixel_in
    CSB_Write(device, Matrix_reg_bias+20,int((feature_in->channel + Tout - 1) / Tout));//pixel_in
	CSB_Write(device, Matrix_reg_bias+26,Out_and_In_Mode);//LN_Out_and_In_Mode

	CSB_Write(device, Matrix_reg_bias+0,0b10000000);
	while (CSB_Read(device, Matrix_reg_bias+1) != 1){	}
	//Kick of the run
}

void FPGA_Run_PosEmb(struct Mapped_Feature* feature_in, struct Mapped_Feature* pos_in, struct Mapped_Feature* feature_out, int Pos_Num, HANDLE device)
{
    int chin_padding_with_tout = ((feature_in->channel + Tout - 1) / Tout) * Tout;
    ...
    CSB_Write(device, Matrix_reg_bias+24,(uint64_t)(pos_in->payload));
    CSB_Write(device, Matrix_reg_bias+31,PosEmb_in_line_stride);
	CSB_Write(device, Matrix_reg_bias+0,0b0100);
	while (CSB_Read(device, Matrix_reg_bias+1) != 1){	}
}
