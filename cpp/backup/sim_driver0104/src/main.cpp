...
int accel_op_test() {
    struct Mapped_Feature* trans = InputAddr(Malloc_Feature(1, 4096, 2048, 0, 0, 16), "transpose_in"); // Hin, Win, CHin: Height -> Hin*Win
    struct Mapped_Weight* weight = transpose(trans, 8);
    ...
    struct Mapped_Feature* output = mvm(data, weight, 1, 8);
